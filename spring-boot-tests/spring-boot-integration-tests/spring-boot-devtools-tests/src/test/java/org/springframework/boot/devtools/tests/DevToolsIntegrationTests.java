...
public class DevToolsIntegrationTests {
	...
	private void initTestRestTemplate() {
	...
	public void addARequestMappingToAnExistingController() throws Exception {
		...
		assertThat(this.template.getForEntity(urlBase + "/two", String.class).getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);

		controller("com.example.ControllerOne").withRequestMapping("one").withRequestMapping("two").build();

		assertThat(this.template.getForObject(urlBase + "/one", String.class)).isEqualTo("one");
		assertThat(this.template.getForObject("http://localhost:" + awaitServerPort() + "/two", String.class)).isEqualTo("two");
	}

	@Test
	public void removeARequestMappingFromAnExistingController() throws Exception {
		assertThat(this.template.getForObject("http://localhost:" + awaitServerPort() + "/one", String.class)).isEqualTo("one");

		controller("com.example.ControllerOne").build();

		assertThat(this.template.getForEntity("http://localhost:" + awaitServerPort() + "/one", String.class).getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}
	...
	public void deleteAController() throws Exception {
		...
		boolean deleted = new File(this.launchedApplication.getClassesDirectory(), "com/example/ControllerOne.class").delete();
